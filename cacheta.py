...
def finalizar_turno():
    ...
    for j in st.session_state.jogadores:
        ...
        # Reset visual da ação para o próximo turno
...
titulos = ["Jogador", "Ordem", "Ação", "Pago?", "Excluir"]
for i, tit in enumerate(titulos): h[i].write(f"**{tit}**")
...
for j in st.session_state.jogadores:
    ...
    # Ordem
    nova_ord = c2.number_input("", value=j["ordem"], key=f"ord_{j['nome']}", label_visibility="collapsed")
    if nova_ord != j["ordem"]:
        j["ordem"] = nova_ord
        salvar()
