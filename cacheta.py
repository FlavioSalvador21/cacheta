...
def carregar_estado():
    if os.path.exists(ARQ):
        with open(ARQ) as f:
            data=json.load(f)
            st.session_state.jogadores=data["jogadores"]
            st.session_state.turno=data["turno"]
            st.session_state.historico_turnos=data["historico_turnos"]
            st.session_state.historico_acoes=data["historico_acoes"]
...
def adicionar():
    ...
    if nome not in st.session_state.stats:
        st.session_state.stats[nome]={"turnos":0,"vitorias":0,"derrotas":0,"desistencias":0,"titulos":0}
...
def reiniciar_turno():
    ...
    for j in st.session_state.jogadores:
        st.session_state[f"acao_{j['nome']}"]=""
